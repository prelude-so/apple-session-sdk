...
final class LogoutTests: XCTestCase {
    ...
    func test_concurrentLogouts_hitRevokeOnce() async throws {
        ...
        try await withThrowingTaskGroup(of: Void.self) { group in
            for _ in 0..<16 { group.addTask { try await fixture.client.logout() } }
    ...
    func test_clearAllStores_partialFailure() async throws {
        let failing = FailingDeleteBackend(
            ...
            failDeleteForService: "so.prelude.session.refresh"
        )
    ...
    func test_partialWipe_andRevokeFailure_surfacesWipeError() async throws {
        ...
        do {
            ...
            guard case .keychainFailure(let status) = error else {
                return XCTFail("expected keychainFailure, got \(error)")
            }
            XCTAssertEqual(
                status,
                errSecIO,
...
final class StubHTTPSession: HTTPSession, @unchecked Sendable {
    struct CannedResponse: Sendable {
        ...
        static let noContent = CannedResponse(statusCode: 204, body: Data(), headers: [:])
        ...
        static func json(
            ...
            var merged: [String: String] = ["Content-Type": "application/json"]
            for (key, value) in headers { merged[key] = value }
            return CannedResponse(statusCode: statusCode, body: data, headers: merged)
        }
    }
...
/// ``PreludeSessionClient/clearAllStores()`` into its
