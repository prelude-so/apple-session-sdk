...
/// ``PreludeSessionClient/adjustedLocalExpiresAt(serverExpiresAt:timeDiffSec:)``
...
final class ClockSkewAdjustmentTests: XCTestCase {
    ...
    func testZeroSkewIsNoOp() {
        XCTAssertEqual(
            PreludeSessionClient.adjustedLocalExpiresAt(serverExpiresAt: server, timeDiffSec: 0),
            server
        )
    }
    ...
    func testPositiveIntegerSkewIsApplied() {
        XCTAssertEqual(
            PreludeSessionClient.adjustedLocalExpiresAt(serverExpiresAt: server, timeDiffSec: 2),
            server + 2
        )
    }
    ...
    func testSubSecondPositiveSkewFloorsToServerExpiresAt() {
        XCTAssertEqual(
            PreludeSessionClient.adjustedLocalExpiresAt(serverExpiresAt: server, timeDiffSec: 0.5),
            server
        )
    }
    ...
    func testFractionalPositiveSkewIsFloored() {
        XCTAssertEqual(
            PreludeSessionClient.adjustedLocalExpiresAt(serverExpiresAt: server, timeDiffSec: 1.9),
            server + 1
        )
    }
    ...
    func testResultNeverOverestimatesExactAdjustedExpiration() {
        ...
        for skew in [-5.0, -2.5, -0.9, -0.1, 0.0, 0.1, 0.9, 2.5, 5.0] {
            let stored = PreludeSessionClient.adjustedLocalExpiresAt(
