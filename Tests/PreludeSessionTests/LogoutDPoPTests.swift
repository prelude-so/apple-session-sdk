...
private struct FailingSigningKeyStore: DPoPKeyStore {
    ...
    var backend: KeychainBackend { inner.backend }
    var nonceStore: DPoPNonceStore { inner.nonceStore }

    func create(domain: String) throws -> DPoPKey {
        FailingSigningKey(real: try inner.create(domain: domain))
    }
    ...
    func getOrCreate(domain: String) throws -> DPoPKey {
        FailingSigningKey(real: try inner.getOrCreate(domain: domain))
    }
    func delete(domain: String) throws { try inner.delete(domain: domain) }
    func getNonce(domain: String) throws -> String? { try inner.getNonce(domain: domain) }
    func setNonce(domain: String, nonce: String) throws {
        try inner.setNonce(domain: domain, nonce: nonce)
    }
    func deleteNonce(domain: String) throws { try inner.deleteNonce(domain: domain) }
}

private struct FailingSigningKey: DPoPKey {
    let real: DPoPKey
    func exportPublicJWK() throws -> [String: String] { try real.exportPublicJWK() }
