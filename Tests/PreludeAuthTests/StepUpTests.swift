...
final class StepUpTests: XCTestCase {
    ...
    /// `expiredChallengeToken` before any /otp/check round-trip,
    /// so the UI can surface "expired, request a fresh one"
    /// distinctly from a mistyped OTP.
    func test_submitStepUpOTP_expiredChallenge_throwsExpiredChallengeToken_noRoundTrip() async throws {
        let fixture = try Fixture.make(domain: domain, baseURL: baseURL, clock: clock)
        try await fixture.prePopulate()
        ...
        do {
            ...
            XCTFail("expected expiredChallengeToken for an expired challenge")
