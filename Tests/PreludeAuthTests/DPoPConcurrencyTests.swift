...
/// ``DPoPKeyStore`` per ``PreludeAuthClient``, so in-process
...
final class DPoPConcurrencyTests: XCTestCase {
    ...
    /// for two ``PreludeAuthClient`` instances in the same
    ...
    func test_getOrCreate_independentInstancesShareOneKeyViaBackend() async throws {
        ...
        var publicKeyFingerprints: [String] = []
        try await withThrowingTaskGroup(of: String.self) { group in
            for index in 0 ..< concurrentCallers {
                let domain = self.domain!
        ...
        XCTAssertEqual(
            uniqueKeys.count,
            1,
            "Independent stores on a shared backend must converge on one key "
                + "via the duplicate-item fallback. Got \(uniqueKeys.count)."
        )
    }
    ...
    func test_setNonce_concurrentFirstTimeWritesAllSucceed() async throws {
        ...
        try await withThrowingTaskGroup(of: Void.self) { group in
            for index in 0 ..< concurrentCallers {
                let domain = self.domain!
                group.addTask {
                    try nonceStore.set(domain: domain, nonce: "nonce-\(index)")
                }
            }
    ...
    func test_setNonce_independentInstancesAllSucceed() async throws {
        ...
        try await withThrowingTaskGroup(of: Void.self) { group in
            for index in 0 ..< concurrentCallers {
                ...
                let store = index.isMultiple(of: 2) ? storeA : storeB
                group.addTask {
                    try store.set(domain: domain, nonce: "nonce-\(index)")
                }
            }
            for try await _ in group {}
        }
