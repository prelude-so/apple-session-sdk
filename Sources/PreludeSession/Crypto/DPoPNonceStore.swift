...
/// ``PreludeSessionClient`` owns one ``DPoPKeyStore`` and therefore
...
/// cross-process races.
struct DPoPNonceStore: Sendable {
    private let backend: KeychainBackend
    private let service: String
    private let lock = NSLock()

    init(backend: KeychainBackend, service: String = "so.prelude.session.dpop-nonce") {
